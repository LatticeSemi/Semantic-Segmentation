...
# Directory arguments
flags.DEFINE_string('train_dataset_dir', '/home/enet/dataset/train',
                    'The dataset directory to find the train images and ground truths.')
flags.DEFINE_string('val_dataset_dir', '/home/enet/dataset/val',
                    'The dataset directory to find the validation images and ground truths.')
flags.DEFINE_string('logdir', './log/Enet', 'The log directory to save your checkpoint and event files.')
...
# Training arguments
flags.DEFINE_boolean('is_training', True, 'Whether user want to train or generate test graph')
...
flags.DEFINE_integer('image_height', 160, "The input height of the images.")
flags.DEFINE_integer('image_width', 160, "The input width of the images.")
...
if is_training:
    image_files = sorted(
        [os.path.join(train_dataset_dir, "images", file) for file in os.listdir(train_dataset_dir + "/images") if
         file.endswith('.jpg')])
    annotation_files = sorted(
        [os.path.join(train_dataset_dir, "labels", file) for file in os.listdir(train_dataset_dir + "/labels") if
         file.endswith('.png')])
    image_val_files = sorted(
        [os.path.join(val_dataset_dir, 'images', file) for file in os.listdir(val_dataset_dir + "/images") if
         file.endswith('.jpg')])
    annotation_val_files = sorted(
        [os.path.join(val_dataset_dir, "labels", file) for file in os.listdir(val_dataset_dir + "/labels") if
         file.endswith('.png')])
    ...
    with tf.Graph().as_default() as graph:
        ...
        if not is_training:
            ...
            with tf.Session() as sess:
                tf.train.write_graph(sess.graph_def, logdir, "model.pbtxt", as_text=True)
            print("Successfully saved test graph as model.pbtxt")
            sys.exit()
        ...
        # Now we need to create a training step function that runs both the train_op, metrics_op and updates the
        # global_step concurrently.
        ...
        image_val = tf.image.rgb_to_grayscale(image_val)
        ...
        # Define your supervisor for running a managed session. Do not run the summary_op automatically or else it
