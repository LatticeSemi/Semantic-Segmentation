...
def ENet_weighing(image_files, num_classes=12):
    ...
    for n in tqdm.tqdm(range(len(image_files))):
    ...
    # print('total_freq: ', total_frequency)
...
def median_frequency_balancing(image_files, num_classes=12):
    ...
    for n in tqdm.tqdm(range(len(image_files))):
        ...
        # For each image sum up the frequency of each label in that image and append to the dictionary if frequency
        # is positive.
        for i in range(num_classes):
            class_mask = np.equal(image, i)
            class_mask = class_mask.astype(np.float32)
    ...
    class_weights[-1] = 0.0
    # print(class_weights, "Thses are te class weights")
    return class_weights
