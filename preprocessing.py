...
def preprocess(image, annotation=None, height=360, width=480):
    ...
    image.set_shape(shape=(height, width, 1))
    image *= 2.0

    if not annotation == None:
        # annotation = tf.image.resize_image_with_crop_or_pad(annotation, height, width)
        annotation = tf.image.resize_image_with_pad(annotation, height, width)
        comp = tf.constant(0.0)
        # annotation = tf.math.not_equal(annotation, comp)
        # annotation = tf.image.convert_image_dtype(annotation, dtype=tf.float32)
