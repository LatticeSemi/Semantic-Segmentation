...
def initial_block(inputs, is_training=True, scope='initial_block'):
    ...
    net_conv0 = vggnet._conv3x3_layer(layer_name=scope + '_conv_bn_0',
                                      inputs=inputs, oc=4, pool_en=False, bias_on=False,
                                      w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                      freeze=False, phase_train=is_training)

    net_conv = vggnet._conv3x3_layer(layer_name=scope + '_conv_bn',
                                     inputs=net_conv0, oc=8, pool_en=True, bias_on=False,
                                     w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                     freeze=False, phase_train=is_training)

    # Max pool branch
    net_pool = vggnet._pooling_layer(layer_name=scope + '_pool',
                                     inputs=net_conv0, size=2, stride=2, padding='SAME')
...
def bottleneck(inputs,
    ...
    with slim.arg_scope([prelu], decoder=decoder):
        ...
        if downsampling:
            ...
            net = vggnet._conv3x3_layer(layer_name=scope + '_conv3_bn_1',
                                        inputs=inputs, oc=reduced_depth, pool_en=True, bias_on=False,
                                        w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                        freeze=False, phase_train=is_training)

            # Second conv block
            net = vggnet._conv3x3_layer(layer_name=scope + '_conv3_bn_2',
                                        inputs=net, oc=reduced_depth, pool_en=False, bias_on=False,
                                        w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                        freeze=False, phase_train=is_training)

            # Final projection with 1x1 kernel
            net = vggnet._conv1x1_layer(layer_name=scope + '_conv1_bn_3',
                                        inputs=net, oc=output_depth, pool_en=False, bias_on=False,
                                        w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                        freeze=False, phase_train=is_training)
            ...
            # net = vggnet._batch_norm_tensor2(name=scope+'_add_bn', x=net, freeze=False, phase_train=is_training) # BatchNorm
            net = vggnet.binary_wrapper(net, a_bin=8, min_rng=0.0, max_rng=2.0)
        ...
        # Everything is the same as a regular bottleneck except for the dilation rate argument
            ...
            net = vggnet._conv1x1_layer(layer_name=scope + '_conv1_bn_1',
                                        inputs=inputs, oc=reduced_depth, pool_en=False, bias_on=False,
                                        w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                        freeze=False, phase_train=is_training)

            # Second conv block --- apply dilated convolution here
            net = vggnet._conv3x3_layer(layer_name=scope + '_dilated_conv3_bn_2',
                                        inputs=net, oc=reduced_depth,
                                        pool_en=False, dilations=dilation_rate, bias_on=False,
                                        w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                        freeze=False, phase_train=is_training)
        ...
        # Everything is the same as a regular bottleneck except for a [5,5] kernel decomposed into two [5,1] then [1,5]
            ...
            net = vggnet._conv3x3_layer(layer_name=scope + '_conv3_bn_2',
                                        ...
                                        w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                        freeze=False, phase_train=is_training)
            net = vggnet._conv3x3_layer(layer_name=scope + '_conv3_bn_3',
                                        inputs=net, oc=reduced_depth, pool_en=False, bias_on=False,
                                        w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                        freeze=False, phase_train=is_training)

            # Final projection with 1x1 kernel
            net = vggnet._conv1x1_layer(layer_name=scope + '_conv1_bn_4',
                                        inputs=net, oc=output_depth, pool_en=False, bias_on=False,
                                        w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                        freeze=False, phase_train=is_training)
        ...
        # Everything is the same as a regular one, except convolution becomes transposed.
            ...
            net_unpool = vggnet._conv1x1_layer(layer_name=scope + '_main_conv1',
                                               inputs=inputs, oc=output_depth, pool_en=False, bias_on=False,
                                               w_bin=8, a_bin=8, min_rng=0.0, max_rng=2.0,
                                               freeze=False, phase_train=is_training)
            ...
            dbg_net1 = net
            net = tf.image.resize_bilinear(net, output_shape[1:3], half_pixel_centers=True, name=scope + '_resize')
            net = vggnet._batch_norm_tensor2(name=scope + '_resize_bn', x=net, freeze=False,
                                             phase_train=is_training)  # BatchNorm
            net = vggnet.binary_wrapper(net, a_bin=8, min_rng=0.0, max_rng=2.0)
            dbg_net2 = net
...
def ENet(inputs,
    ...
    inputs_net = tf.transpose(inputs, perm=[0, 3, 1, 2])
    ...
    with tf.variable_scope(scope, reuse=reuse):
        ...
        with slim.arg_scope([initial_block, bottleneck], is_training=is_training), \
            ...
            # ===================STAGE ONE=======================
            net, pooling_indices_1, inputs_shape_1 = bottleneck(net, output_depth=40, filter_size=3,
                                                                regularizer_prob=0.01, downsampling=True,
                                                                projection_ratio=1, scope='bottleneck1_0')
            net = bottleneck(net, output_depth=40, filter_size=3, regularizer_prob=0.01, scope='bottleneck1_1')
            net = bottleneck(net, output_depth=40, filter_size=3, regularizer_prob=0.01, scope='bottleneck1_2')
            net = bottleneck(net, output_depth=40, filter_size=3, regularizer_prob=0.01, scope='bottleneck1_3')
            net = bottleneck(net, output_depth=40, filter_size=3, regularizer_prob=0.01, scope='bottleneck1_4')
            ...
            with slim.arg_scope([bottleneck], regularizer_prob=0.1):
                net, pooling_indices_2, inputs_shape_2 = bottleneck(net, output_depth=80, filter_size=3,
            ...
            with slim.arg_scope([bottleneck], regularizer_prob=0.1, decoder=True):
                ...
                net, dbg_net1, dbg_net2 = bottleneck(net, output_depth=40, filter_size=3, upsampling=True,
                                                     pooling_indices=pooling_indices_2, output_shape=inputs_shape_2,
                                                     scope=bottleneck_scope_name + '_0')

                dbg_net = dbg_net2
                dbg_net = tf.transpose(dbg_net, perm=[0, 3, 1, 2])

                # Perform skip connections here
                if skip_connections:
                    net = tf.add(net, net_two, name=bottleneck_scope_name + '_skip_connection')

                net = bottleneck(net, output_depth=40, filter_size=3, scope=bottleneck_scope_name + '_1')
                net = bottleneck(net, output_depth=40, filter_size=3, scope=bottleneck_scope_name + '_2')

                # ===================STAGE FIVE========================
                bottleneck_scope_name = "bottleneck" + str(i + 2)

                net, _, _ = bottleneck(net, output_depth=12, filter_size=3, upsampling=True,
                ...
                net = bottleneck(net, output_depth=12, filter_size=3, scope=bottleneck_scope_name + '_1')

            # =============FINAL CONVOLUTION=============
            # logits = slim.conv2d_transpose(net, num_classes, [2,2], stride=2, scope='fullconv')
            net_shape = net.get_shape().as_list()
            resize_shape = tf.convert_to_tensor([2 * net_shape[1], 2 * net_shape[2]])
            net = tf.image.resize_bilinear(net, resize_shape, half_pixel_centers=True, name='resize')
            net = vggnet._batch_norm_tensor2(name=scope + '_resize_bn', x=net, freeze=False,
                                             phase_train=is_training)  # BatchNorm
            net = vggnet.binary_wrapper(net, a_bin=8, min_rng=0.0, max_rng=2.0)
            # tf.summary.histogram('before_fullconv', net)
            logits = vggnet._conv_layer(layer_name=scope + '_fullconv',
                                        inputs=net, filters=num_classes, size=3, stride=1, dilations=1,
                                        xavier=False, padding='SAME', stddev=0.0001, freeze=False,
                                        relu=False, w_bin=8, bias_on=False)
            # tf.summary.histogram('after_fullconv', logits)
            probabilities = tf.nn.softmax(logits, name='logits_to_softmax')

        # return logits, probabilities, dbg_net
